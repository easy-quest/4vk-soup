...
def get_wall_posts(group_name):
    url = f"https://api.vk.com/method/wall.get?domain={group_name}&count=40&access_token={my_secret}&v=5.131 "
    ...
    if not os.path.exists(f"{group_name}/exist_posts_{group_name}.txt"):
        ...
        for post in posts:
            ...
            def download_img(url, post_id, group_name):
                ...
                with open(f"{group_name}/files/{post_id}.jpg",
                          "wb") as img_file:
            ...
            def download_video(url, post_id, group_name):
                ...
                try:
                    ydl_opts = {
                        "outtmpl": f"{group_name}/video_files/{post_id}"
                    }
                    with youtube_dl.YoutubeDL(ydl_opts) as ydl:
                        ...
                        if video_duration > 300:
                            ...
                            print(
                                f"Видео длится {video_duration} секунд. Сохраняем видео..."
                            )
                            ydl.download([url])
                except Exception:
            ...
            try:
                if "attachments" in post:
                    ...
                    photo_quality = [
                        "photo_2560", "photo_1280", "photo_807", "photo_604",
                        "photo_130", "photo_75"
                    ]
                        ...
                        if post[0]["type"] == "photo":
                            ...
                            for pq in photo_quality:
                                if pq in post[0]["photo"]:
                                    ...
                                    print(post_photo)
                                    download_img(post_photo, post_id,
                                                 group_name)
                                    break
                        # забираем видео
                            ...
                            video_get_url = f"https://api.vk.com/method/video.get?videos={video_owner_id}_{video_post_id}_{video_access_key}&access_token={my_secret}&v=5.131"
                            ...
                            if post_item_photo["type"] == "photo":
                                for pq in photo_quality:
                                    if pq in post_item_photo["photo"]:
                                        ...
                                        print(post_photo)
                                        post_id_counter = str(
                                            post_id) + f"_{photo_post_count}"
                                        download_img(post_photo,
                                                     post_id_counter,
                                                     group_name)
                                        photo_post_count += 1
                                        break
                                ...
                                req = requests.get(video_get_url)
                                res = req.json()
                                video_url = res["response"]["items"][0][
                                    "player"]
                                print(video_url)
                                post_id_counter = str(
                                    post_id) + f"_{photo_post_count}"
                                download_video(video_url, post_id_counter,
                                               group_name)
                                photo_post_count += 1
                            else:
                                print("Либо линк, либо аудио, либо репост...")
...
def main():
    group_name = 'baraholka_ustlabinsk'
